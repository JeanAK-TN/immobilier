...
<section class="grid gap-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <p class="text-xs font-medium uppercase tracking-wider text-gray-400">{{ __('Référence') }}</p>
        ...
        <dl class="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            <div>
                <dt class="text-xs font-medium uppercase tracking-wider text-gray-400">{{ __('Montant') }}</dt>
                <dd class="mt-1.5 text-lg font-bold text-gray-900"><x-money :amount="$paiement->montant" /></dd>
            </div>

            <div>
                <dt class="text-xs font-medium uppercase tracking-wider text-gray-400">{{ __('Mode') }}</dt>
        ...
        <div class="mt-6 grid gap-4 rounded-xl bg-slate-50 p-5 text-sm md:grid-cols-2">
            <div>
                <p class="text-xs font-medium uppercase tracking-wider text-gray-400">{{ __('Date d\'enregistrement') }}</p>
                <p class="mt-1.5 font-semibold text-gray-900">{{ $paiement->created_at?->translatedFormat('d F Y H:i') }}</p>
            </div>

            <div>
                <p class="text-xs font-medium uppercase tracking-wider text-gray-400">{{ __('Commentaire système') }}</p>
