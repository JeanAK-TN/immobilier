                ...
                <section class="rounded-3xl border border-blue-200 bg-blue-50 p-6 shadow-sm">
                    ...
                    <dl class="mt-5 grid gap-4 md:grid-cols-2">
                        <div class="rounded-2xl bg-white px-4 py-4 shadow-sm">
                            <dt class="text-xs uppercase tracking-wide text-gray-500">{{ __('Adresse e-mail') }}</dt>
                            <dd class="mt-2 text-sm font-semibold text-gray-900">{{ session('identifiants_locataire.email') }}</dd>
                        </div>

                        <div class="rounded-2xl bg-white px-4 py-4 shadow-sm">
                            <dt class="text-xs uppercase tracking-wide text-gray-500">{{ __('Mot de passe temporaire') }}</dt>
                            <dd class="mt-2 text-sm font-semibold text-gray-900">{{ session('identifiants_locataire.mot_de_passe_temporaire') }}</dd>
                        </div>
                    </dl>
                </section>
            @endif
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
                <section class="grid gap-6">
                    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        <dl class="mt-6 grid gap-6 md:grid-cols-2">
                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Prénom') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $locataire->prenom }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Nom') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $locataire->nom }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Adresse e-mail') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $locataire->email }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Téléphone') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $locataire->telephone ?: __('Non renseigné') }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Créé le') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $locataire->created_at->translatedFormat('d F Y') }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Compte utilisateur') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $locataire->user->name }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-6 grid gap-4 md:grid-cols-2">
                            <div class="rounded-2xl bg-gray-50 p-5">
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Nombre de contrats') }}</p>
                                <p class="mt-2 text-2xl font-semibold text-gray-900">{{ $locataire->contrats_count }}</p>
                            </div>

                            <div class="rounded-2xl bg-gray-50 p-5">
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Contrats actifs') }}</p>
                                <p class="mt-2 text-2xl font-semibold text-gray-900">{{ $locataire->contrats_actifs_count }}</p>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <aside class="grid gap-6">
                    <section class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-5 grid gap-4 rounded-2xl bg-gray-50 p-5 text-sm text-gray-700">
                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('État du compte') }}</p>
                            ...
                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Première connexion') }}</p>
                                <p class="mt-2 font-semibold">
                        ...
                        <form method="POST" action="{{ route('proprietaire.locataires.activation', $locataire) }}" class="mt-5">
                            ...
                            <button
                                type="submit"
                                class="inline-flex items-center rounded-md px-4 py-2 text-sm font-semibold shadow-sm transition {{ $locataire->compteActif() ? 'bg-red-600 text-white hover:bg-red-500' : 'bg-emerald-600 text-white hover:bg-emerald-500' }}"
