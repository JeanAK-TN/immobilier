        ...
        <div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <a href="{{ route('proprietaire.biens.index') }}"
                   class="group rounded-3xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide text-gray-500">Biens</p>
                ...
                <a href="{{ route('proprietaire.contrats.index', ['statut' => 'actif']) }}"
                   class="group rounded-3xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide text-gray-500">Contrats actifs</p>
                    ...
                    @else
                        <p class="mt-3 text-sm text-gray-400">Aucune signature en attente</p>
                    @endif
                </a>
                ...
                <a href="{{ route('proprietaire.paiements.index', ['periode' => now()->format('Y-m')]) }}"
                   class="group rounded-3xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide text-gray-500">
                        Paiements — {{ ucfirst(now()->translatedFormat('F')) }}
                    </p>
                    <p class="mt-3 text-4xl font-bold text-gray-900">{{ $paiementsMoisCount }}</p>
                    @if ($paiementsMoisMontant > 0)
                        <p class="mt-3 text-sm text-emerald-600 font-medium">
                            <x-money :amount="$paiementsMoisMontant" /> reçus
                        </p>
                    @else
                        <p class="mt-3 text-sm text-gray-400">Aucun paiement ce mois</p>
                    @endif
                </a>
                ...
                <a href="{{ route('proprietaire.tickets.index', ['statut' => 'ouvert']) }}"
                   class="group rounded-3xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md">
                    <p class="text-xs font-medium uppercase tracking-wide text-gray-500">Tickets ouverts</p>
                    <p class="mt-3 text-4xl font-bold {{ $ticketsOuverts > 0 ? 'text-amber-600' : 'text-gray-900' }}">
                        {{ $ticketsOuverts }}
                    </p>
                    <p class="mt-3 text-sm text-gray-400">
                        {{ $ticketsOuverts > 0 ? 'En attente de traitement' : 'Aucune demande en cours' }}
                    </p>
                </a>
            ...
            <div class="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(260px,1fr)]">
                ...
                <section class="rounded-3xl border border-gray-200 bg-white shadow-sm">
                    <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                        <h3 class="font-semibold text-gray-900">Derniers contrats</h3>
                    ...
                    @if ($derniersContrats->isEmpty())
                        <div class="px-6 py-10 text-center text-sm text-gray-400">
                            Aucun contrat créé pour l'instant.
                        </div>
                    @else
                                       ...
                                       class="flex items-center justify-between px-6 py-4 transition hover:bg-gray-50">
                                        ...
                                        <div class="ml-4 shrink-0 text-right">
                                            <span @class([
                                                ...
                                                'bg-emerald-100 text-emerald-700' => $contrat->statut->value === 'actif',
                                                'bg-amber-100 text-amber-700' => $contrat->statut->value === 'en_attente',
                                                'bg-gray-100 text-gray-600' => ! in_array($contrat->statut->value, ['actif', 'en_attente']),
                                            ])>
                ...
                <aside class="flex flex-col gap-3">
                    <h3 class="text-sm font-semibold uppercase tracking-wide text-gray-500">Accès rapide</h3>

                    <a href="{{ route('proprietaire.biens.create') }}"
                       class="flex items-center gap-3 rounded-2xl border border-gray-200 bg-white px-4 py-3.5 text-sm font-semibold text-gray-700 shadow-sm transition hover:bg-gray-50">
                        <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-gray-100 text-gray-600">+</span>
                        Ajouter un bien
                    </a>
                    ...
                    @if ($contratsEnAttente > 0)
                        <a href="{{ route('proprietaire.contrats.index', ['statut' => 'en_attente']) }}"
                           class="flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3.5 text-sm font-semibold text-amber-700 shadow-sm transition hover:bg-amber-100">
                            <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-100 text-amber-600">!</span>
