                ...
                <section class="grid gap-6">
                    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Type') }}</p>
                                <p class="mt-2 text-base font-semibold text-gray-900">{{ $bien->type->label() }}</p>
                            </div>

                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Statut déclaré') }}</p>
                                <p class="mt-2 text-base font-semibold text-gray-900">{{ $bien->statut->label() }}</p>
                            </div>

                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Occupation actuelle') }}</p>
                                <p class="mt-2 text-base font-semibold text-gray-900">{{ $bien->occupationLabel() }}</p>
                            </div>

                            <div>
                                <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Photos') }}</p>
                                <p class="mt-2 text-base font-semibold text-gray-900">{{ $bien->photos_count }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        <dl class="mt-6 grid gap-6 md:grid-cols-2">
                            <div>
                                ...
                                <dd class="mt-2 text-sm text-gray-900">{{ $bien->adresse }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Ville') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $bien->ville }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Pays') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $bien->pays }}</dd>
                            </div>

                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('Créé le') }}</dt>
                                <dd class="mt-2 text-sm text-gray-900">{{ $bien->created_at->translatedFormat('d F Y') }}</dd>
                            </div>

                            <div class="md:col-span-2">
                                <dt class="text-sm font-medium text-gray-500">{{ __('Description') }}</dt>
                                <dd class="mt-2 text-sm leading-6 text-gray-900">
                    ...
                    <div class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        @if ($bien->photos->isEmpty())
                            <div class="mt-6 rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-6 py-12 text-center text-sm text-gray-500">
                                {{ __('Aucune photo n\'a encore été ajoutée à ce bien.') }}
                            </div>
                        @else
                            <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                                @foreach ($bien->photos as $photo)
                                    <figure class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
                                        <img
                                            src="{{ $photo->url() }}"
                                        ...
                                        <figcaption class="grid gap-1 p-4">
                    ...
                    <section class="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                        ...
                        @if ($contratActif)
                            <div class="mt-5 grid gap-4 rounded-2xl bg-amber-50 p-5 text-sm text-amber-900">
                                <div>
                                    <p class="text-xs uppercase tracking-wide text-amber-700">{{ __('Contrat actif') }}</p>
                                    <p class="mt-2 font-semibold">{{ __('Oui') }}</p>
                                </div>

                                <div>
                                    <p class="text-xs uppercase tracking-wide text-amber-700">{{ __('Locataire') }}</p>
                                    <p class="mt-2 font-semibold">{{ $contratActif->locataire->nomComplet() }}</p>
                                </div>

                                <div>
                                    <p class="text-xs uppercase tracking-wide text-amber-700">{{ __('Début du contrat') }}</p>
                                    <p class="mt-2 font-semibold">{{ $contratActif->date_debut->translatedFormat('d F Y') }}</p>
                                </div>
                            </div>
                        @else
                            <div class="mt-5 rounded-2xl bg-emerald-50 p-5 text-sm text-emerald-900">
                                <p class="font-semibold">{{ __('Aucun contrat actif') }}</p>
                                <p class="mt-2">{{ __('Ce bien apparaît actuellement comme libre de tout contrat actif.') }}</p>
                            </div>
                        @endif
                    </section>
